import { useRef, useState } from 'react'
import { motion, useInView } from 'framer-motion'
import { useTranslation } from 'react-i18next'
import { WA_URL, EMAIL_URL, EMAIL_ADDRESS } from '../config/constants'

export default function Contact() {
  const { t } = useTranslation()
  const ref = useRef()
  const inView = useInView(ref, { once: true })
  const [form, setForm] = useState({ name: '', company: '', message: '' })

  const handleSubmit = (e) => {
    e.preventDefault()
    const body    = encodeURIComponent(`Nombre: ${form.name}\nEmpresa: ${form.company}\n\n${form.message}`)
    const subject = encodeURIComponent('Consulta desde KARDRA')
    window.location.href = `mailto:${EMAIL_ADDRESS}?subject=${subject}&body=${body}`
  }

  return (
    <div className="bg-[#19171b]">
      <section ref={ref} className="relative min-h-screen border-b border-[#2e2c30] overflow-hidden">

        {/* Left gold accent */}
        <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#d29f22]" />

        {/* Background large text */}
        <div className="absolute right-0 bottom-0 pointer-events-none select-none overflow-hidden opacity-[0.04]">
          <span className="font-display text-[25vw] text-[#f0ede8] leading-none">K</span>
        </div>

        <div className="px-6 md:px-12 lg:px-20 py-20 md:py-28 grid grid-cols-1 lg:grid-cols-2 gap-16 md:gap-24">

          {/* Left */}
          <div>
            <motion.div
              initial={{ opacity: 0, x: -20 }}
              animate={inView ? { opacity: 1, x: 0 } : {}}
              className="flex items-center gap-4 mb-10"
            >
              <div className="w-6 h-[2px] bg-[#d29f22]" />
              <span className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.3em] uppercase text-[#d29f22] border border-[#d29f22]/40 px-3 py-1 inline-flex items-center gap-2">
                <span className="w-1.5 h-1.5 bg-[#d29f22]" />
                CONTACTO
              </span>
            </motion.div>

            <motion.h1
              initial={{ opacity: 0, y: 30 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.15, duration: 0.8 }}
              className="font-display text-[clamp(3rem,7vw,8rem)] mb-6 leading-[0.88]"
            >
              {t('contact.title')}
            </motion.h1>

            <motion.p
              initial={{ opacity: 0 }}
              animate={inView ? { opacity: 1 } : {}}
              transition={{ delay: 0.3 }}
              className="text-[#6a6868] text-lg mb-14 max-w-sm leading-relaxed"
            >
              {t('contact.text')}
            </motion.p>

            {/* Direct contact */}
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={inView ? { opacity: 1, y: 0 } : {}}
              transition={{ delay: 0.45 }}
              className="flex flex-col gap-0 border-t border-[#2e2c30]"
            >
              <a
                href={WA_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="group flex items-center gap-5 py-6 border-b border-[#2e2c30] hover:bg-[#252628] px-4 -mx-4 transition-colors duration-200"
              >
                <div className="w-10 h-10 border border-[#d29f22]/40 flex items-center justify-center flex-shrink-0 group-hover:bg-[#d29f22]/10 transition-colors">
                  <svg viewBox="0 0 24 24" className="w-4 h-4 fill-[#d29f22]">
                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.464 3.488"/>
                  </svg>
                </div>
                <div>
                  <p className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.2em] uppercase text-[#6a6868] mb-0.5">WhatsApp</p>
                  <p className="text-[#f0ede8] text-sm font-['Space_Grotesk'] font-600">+593 98 418 7556</p>
                </div>
                <span className="ml-auto text-[#d29f22] opacity-0 group-hover:opacity-100 transition-opacity">→</span>
              </a>

              <a
                href={EMAIL_URL}
                className="group flex items-center gap-5 py-6 border-b border-[#2e2c30] hover:bg-[#252628] px-4 -mx-4 transition-colors duration-200"
              >
                <div className="w-10 h-10 border border-[#d29f22]/40 flex items-center justify-center flex-shrink-0 group-hover:bg-[#d29f22]/10 transition-colors">
                  <svg viewBox="0 0 24 24" className="w-4 h-4" fill="none" stroke="#d29f22" strokeWidth="1.5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M21.75 6.75v10.5a2.25 2.25 0 01-2.25 2.25h-15a2.25 2.25 0 01-2.25-2.25V6.75m19.5 0A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25m19.5 0v.243a2.25 2.25 0 01-1.07 1.916l-7.5 4.615a2.25 2.25 0 01-2.36 0L3.32 8.91a2.25 2.25 0 01-1.07-1.916V6.75" />
                  </svg>
                </div>
                <div>
                  <p className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.2em] uppercase text-[#6a6868] mb-0.5">Email</p>
                  <p className="text-[#f0ede8] text-sm font-['Space_Grotesk'] font-600">daesgaar@gmail.com</p>
                </div>
                <span className="ml-auto text-[#d29f22] opacity-0 group-hover:opacity-100 transition-opacity">→</span>
              </a>
            </motion.div>
          </div>

          {/* Right: form */}
          <motion.form
            initial={{ opacity: 0, x: 30 }}
            animate={inView ? { opacity: 1, x: 0 } : {}}
            transition={{ delay: 0.4, duration: 0.8 }}
            onSubmit={handleSubmit}
            className="flex flex-col gap-6"
          >
            {/* Form header */}
            <div className="flex items-center gap-3 mb-2">
              <div className="w-6 h-[2px] bg-[#d29f22]" />
              <span className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.3em] uppercase text-[#6a6868]">
                FORMULARIO
              </span>
            </div>

            {[
              { key: 'name',    type: 'text' },
              { key: 'company', type: 'text' },
            ].map(({ key, type }) => (
              <div key={key} className="flex flex-col gap-2">
                <label className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.2em] uppercase text-[#6a6868]">
                  {t(`contact.form.${key}`)}
                </label>
                <input
                  type={type}
                  value={form[key]}
                  onChange={(e) => setForm(p => ({ ...p, [key]: e.target.value }))}
                  className="bg-transparent border border-[#2e2c30] px-4 py-3.5 text-[#f0ede8] text-sm focus:outline-none focus:border-[#d29f22] transition-colors placeholder:text-[#2e2c30]"
                />
              </div>
            ))}

            <div className="flex flex-col gap-2">
              <label className="text-[10px] font-['Space_Grotesk'] font-600 tracking-[0.2em] uppercase text-[#6a6868]">
                {t('contact.form.message')}
              </label>
              <textarea
                rows={5}
                value={form.message}
                onChange={(e) => setForm(p => ({ ...p, message: e.target.value }))}
                className="bg-transparent border border-[#2e2c30] px-4 py-3.5 text-[#f0ede8] text-sm focus:outline-none focus:border-[#d29f22] transition-colors resize-none placeholder:text-[#2e2c30]"
              />
            </div>

            <button
              type="submit"
              className="bg-[#5d0018] text-[#f0ede8] px-8 py-4 font-['Space_Grotesk'] font-700 text-[11px] tracking-[0.2em] uppercase hover:bg-[#d29f22] hover:text-[#19171b] transition-all duration-250 self-start border-2 border-[#5d0018] hover:border-[#d29f22]"
            >
              {t('contact.form.send')}
            </button>
          </motion.form>
        </div>
      </section>
    </div>
  )
}
